...
def getPicture():
	...
	picam2.start()
	time.sleep(2)
	picam2.capture_file("../showPhoto/static/test.png")
	picam2.stop()
	# reading image 
   ...
   # here we are ignoring first counter because  
		...
		approx = cv2.approxPolyDP(contour, 0.038 * cv2.arcLength(contour, True), True) 
        ...
        # else:
        #     cv2.putText(board, f'{len(approx)}', (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
        #     print("you fail pussy boy")
    # displaying the image after drawing contours
	cv2.imwrite('../showPhoto/static/converted.png', board)
